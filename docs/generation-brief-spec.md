S2-00 — Generation Brief Contract v1
...
القرار
Campaign Goal
Single select أو بطاقات اختيار
نعم
خيارات جاهزة + custom
Target Audience
Hybrid: خيارات مساعدة + حقول كتابة
نعم
لا تجعله dropdown واحدًا
Content Type
Single select أو بطاقات
نعم
خيارات جاهزة + custom
Core Idea
Textarea
نعم
كتابة حرة مع مثال واضح
Voice/Tone
Single select + خيار modifiers اختياري
نعم
نبرة أساسية واحدة، ويمكن إضافة صفة أو صفتين
Optional Notes
Textarea
لا
كتابة حرة بالكامل
Platform/Format
الموجود حاليًا platform_preset
نعم
يبقى منفصلًا عن content_type
Logo Mode
الموجود حاليًا logo_mode
نعم
يبقى منفصلًا
Provider
الموجود حاليًا provider
نعم
يبقى منفصلًا
...
تعريف الناس بالعلامة أو هويتها
product_launch
إطلاق منتج أو خدمة
منتج جديد أو تحديث مهم
product_showcase
إبراز منتج أو خدمة
إظهار المزايا بصريًا
promotion_offer
عرض أو خصم
حملة تحويلية أو سعرية
sales_conversion
زيادة المبيعات
توجيه المستخدم لاتخاذ إجراء شراء
lead_generation
جمع عملاء محتملين
نموذج تواصل أو طلب استشارة
engagement
زيادة التفاعل
سؤال، رأي، مشاركة، أو تفاعل اجتماعي
education
التوعية أو التعليم
شرح فكرة أو نصيحة أو معلومة
announcement
إعلان خبر أو تحديث
افتتاح، تغيير، إطلاق، أو خبر
event_registration
الترويج لفعالية
مؤتمر، ورشة، افتتاح، أو مناسبة
seasonal_campaign
حملة موسمية
رمضان، العيد، الصيف، العودة للمدارس
social_proof
إبراز الثقة وتجربة العملاء
testimonial أو review أو نتائج
custom
هدف آخر
يفتح حقلًا صغيرًا للكتابة
...
إلزامي؟
Audience segment
single أو multi-select محدود
نعم
Location/market
searchable country/region select
لا، لكنه مفضل
Age range
single select
لا
...
رواد الأعمال
b2b_decision_makers
أصحاب القرار في الشركات
marketers_creators
المسوقون وصناع المحتوى
professionals
المهنيون والموظفون
students
الطلاب
...
beauty_fashion_audience
جمهور الموضة والجمال
food_hospitality_audience
جمهور الطعام والضيافة
technology_users
مستخدمو التقنية
local_community
المجتمع المحلي
...
اقتباس أو نصيحة
infographic
إنفوغراف أو شرح بصري
recruitment
توظيف أو ثقافة شركة
social_proof
دليل اجتماعي أو نتائج
custom
نوع آخر
...
visual_format
single select
يفرق بين product hero وlifestyle وposter وflat lay
style_tags
multi-select بحد أقصى 3
يضيف minimal/editorial/cinematic وغيرها
output_language
single select
مهم عند دعم العربية والإنجليزية
cta_type
single select + custom
يوجه التصميم نحو Shop Now أو Learn More
season_or_context
select + text
مفيد للمواسم وGoogle Trends لاحقًا




إذا كان هدفكم تحسين الصور بسرعة، أضيفوا visual_format وoutput_language إلى تصميم Sprint 2، لكن لا تغيروا الحقول الأساسية الستة قبل موافقة الفريق.
...
في عقد S2-00 لا نرسل visual_format أو output_language. يمكن إضافتهما في Sprint لاحق بعد اختبار الـMVP، ويجب إبقاؤهما ضمن قسم Future Fields فقط.

9. قواعد مهمة لتجنب dropdown سيئ

لا تجعلوا القائمة تحتوي على عشرات الخيارات بلا بحث. استخدموا Select للمجموعات القصيرة، وCombobox للقوائم الطويلة، وchips للصفات المتعددة. يجب أن يكون لكل خيار label عربي مترجم وkey داخلي ثابت. أضيفوا دائمًا custom عندما تكون القائمة غير قادرة على تغطية كل الحالات.
...
اعتمدوا في S2-00 هذه النسخة:

المستوى
الحقول
Required dropdowns
campaign_goal, content_type, voice_tone
Hybrid structured + free text
target_audience
Required free text
core_idea
Optional free text
optional_notes, text_to_include
Existing generation controls
provider, platform_preset, logo_mode
Future optional fields
visual_format, style_tags, output_language, cta_type
...
المسؤولية
Frontend
جمع القيم، عرض labels، التحقق الأولي، وإرسال object منظم فقط
Backend
التحقق من schema، تحميل Brand Kit، وتمرير brief إلى prompt composer
AI Engineer
بناء prompt deterministic ودمج brief مع Brand Kit وPlatform وLogo rules
Database
لا migration في S2؛ يمكن حفظ core_idea مؤقتًا في history
...
•
تم اعتماد JSON نفسه من Frontend وBackend وAI Engineer.

•
كل enum له key إنجليزي ثابت وlabel منفصل قابل للترجمة.

•
الحقول المطلوبة والاختيارية وحدود النصوص موثقة.

•
سلوك custom موثق ويحتاج نصًا إضافيًا.

•
يوجد مثال صحيح ومثال مرفوض على الأقل.

•
لا يحتوي هذا العقد على visual_format أو output_language كحقول تنفيذية.

•
لا يتضمن هذا الـcommit تعديلًا على production code.

13. الخطوة التالية

بعد إغلاق S2-00، يبدأ Frontend بـS2-FE-01 لبناء types وstate، ويبدأ Backend بـS2-BE-01 لبناء Pydantic models. لا يبدأ ربط الـAPI أو تعديل prompt_composer.py قبل أن يتطابق التنفيذ مع هذا الملف.
